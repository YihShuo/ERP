...
begin
  ...
  begin
    ...
    begin
      if edit1.Text='' then
      begin
         Showmessage('Vui long chon ma ke. Abort');
         abort;
      end else
      begin
      Active:=false;
      SQL.Clear;
      sql.Add('Update KCRKScan_RFSS set RemQty=(Case when RemQty is null then Qty else RemQty end), CFMID='''+main.Edit1.Text+''', CFMDATE=getdate(), MAKE= '''+Edit1.Text+''' ');
