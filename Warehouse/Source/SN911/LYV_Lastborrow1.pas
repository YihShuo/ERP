...
var
  LYV_Lastborrow: TLYV_Lastborrow;
  sdate,edate,NDate:Tdate;

implementation

uses main1,LastBorrow_Dep1, LastBorrow_CL1 , LYV_Lastborrow_PrintA41,
     LYV_Lastborrow_S1, LastBorrow_RY1, FunUnit;

{$R *.dfm}

procedure TLYV_Lastborrow.FormClose(Sender: TObject;
...
procedure TLYV_Lastborrow.btn2Click(Sender: TObject);
var y,m:word;
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.add('select LTLL.*,BDepartment.DepName');
    sql.add('from LTLL');
    sql.add('left join BDepartment on LTLL.DepID=BDepartment.ID');
    sql.add('where LTLL.SB=''L'' ');
    IF Edt1.TEXT<>'' THEN
...
procedure TLYV_Lastborrow.BB1Click(Sender: TObject);
begin
  pgcPC1.ActivePage:=TSTS1;
  pnl2.Visible:=true;
  Edt1.SetFocus;
...
procedure TLYV_Lastborrow.BB7Click(Sender: TObject);
begin
  close;
end;

procedure TLYV_Lastborrow.BBT1Click(Sender: TObject);
begin
  DelMas.First;
end;

procedure TLYV_Lastborrow.BBT2Click(Sender: TObject);
begin
  DelMas.Prior;
end;

procedure TLYV_Lastborrow.BBT3Click(Sender: TObject);
begin
  DelMas.Next;
end;

procedure TLYV_Lastborrow.BBT4Click(Sender: TObject);
begin
  DelMas.Last;
end;


procedure TLYV_Lastborrow.FormCreate(Sender: TObject);
...
begin
  ...
  begin
    ...
    SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH='''+main.Edit2.Text+''' and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
    SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
    SQL.Add('  Union All ');
    SQL.Add('  select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH='''+main.Edit2.Text+''' and KCYEAR='''+ayear+''' and KCMonth='''+amonth+''' ) as tmpRKTLFL ');
    SQL.Add('  group by CKBH,CLBH,SIZ ) as NowKC');
    SQL.Add('left join ( ');
    SQL.Add('   select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH='''+main.Edit2.Text+''' and KCYEAR='''+ayear+''' and KCMonth='''+amonth+''' ) as LastKC on NowKC.CKBH=LastKC.CKBH and NowKC.CLBH=LastKC.CLBH and NowKC.SIZ=LastKC.SIZ');
    SQL.Add('left join ( ');
    SQL.Add('  select LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK ');
    SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH='''+main.Edit2.Text+''' and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
    ...
    SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.CKBH='''+main.Edit2.Text+''' and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
...
procedure TLYV_Lastborrow.DelMasAfterOpen(DataSet: TDataSet);
begin
  BB2.Enabled:=true;
  BB3.Enabled:=true;
  BB4.Enabled:=true;
...
procedure TLYV_Lastborrow.BB2Click(Sender: TObject);
begin
  with DelMas do
  begin
      requestlive:=true;
...
procedure TLYV_Lastborrow.BB3Click(Sender: TObject);
begin
  ...
  BB5.Enabled:=true;
  BB6.Enabled:=true;
  //dbgrdh3.columns[2].ButtonStyle:=cbsEllipsis;
end;

procedure TLYV_Lastborrow.BB4Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TLYV_Lastborrow.BB5Click(Sender: TObject);
var y,m,a:string;
    i:integer;
begin
...
procedure TLYV_Lastborrow.btnBBTT1Click(Sender: TObject);
begin
  LastBorrow_Dep:=TLastborrow_Dep.Create(self);
  Lastborrow_Dep.Label3.Caption := 'LastBorrow';
  Lastborrow_Dep.Show;
end;

procedure TLYV_Lastborrow.BB6Click(Sender: TObject);
begin
  with DelMas do
  begin
    active:=false;
...
procedure TLYV_Lastborrow.DBGridh3EditButtonClick(Sender: TObject);
begin
  LastBorrow_Dep:=TLastBorrow_Dep.Create(self);
  LastBorrow_Dep.Show;
end;

procedure TLYV_Lastborrow.DelDetAfterOpen(DataSet: TDataSet);
begin
  BD2.Enabled:=true;
  BD3.Enabled:=true;
  BD4.Enabled:=true;
...
procedure TLYV_Lastborrow.pgcPC1Change(Sender: TObject);
begin
...
procedure TLYV_Lastborrow.BD7Click(Sender: TObject);
begin
close;
end;

procedure TLYV_Lastborrow.BD2Click(Sender: TObject);
begin
...
procedure TLYV_Lastborrow.BD3Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TLYV_Lastborrow.BD5Click(Sender: TObject);
var i:integer;
  Bookmark1:   string;
begin
  try
                    ...
                    begin
                          ...
                          //自動新增尺寸明細
                           ...
                           begin
                              ...
                              sql.add('select '''+DelDet.fieldbyname('LLNo').Value+''',CLBH,''LAST'' ,SIZ,''0'',''0'',Qty ');
                              sql.add('       ,getdate() '  );
                              ...
                              sql.add('                                    and LTLLSS.LLNo='+''''+DelDet.FieldByName('LLNO').Value+''''+' and LTLLSS.CLBH='+''''+DelDet.FieldByName('CLBH').Value+'''' );
                              sql.add('                             GROUP BY LLNo,CLBH),0) ');
                              sql.add('where LTLLS.LLNo='+''''+DelDet.FieldByName('LLNo').Value+''''+ ' and LTLLS.CLBH='+''''+DelDet.FieldByName('CLBH').Value+'''' );
                              funcObj.WriteErrorLog(sql.Text);
                              execsql;
                              active:=false;
                              close;
...
procedure TLYV_Lastborrow.BD4Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TLYV_Lastborrow.BD6Click(Sender: TObject);
begin
...
procedure TLYV_Lastborrow.btnBBTT2Click(Sender: TObject);
begin
  if messagedlg('Phom giay RY hay mat tu ?',mtinformation,[mbYes,mbNo],0)=mrYes then
  begin
    LastBorrow_RY:=TLastBorrow_RY.create(self);
    LastBorrow_RY.show;
  end else
  begin
    LastBorrow_CL:=TLastBorrow_CL.create(self);
    LastBorrow_CL.show;
  end;

end;

procedure TLYV_Lastborrow.dbgrdh4EditButtonClick(Sender: TObject);
begin
  btnBBTT2Click(nil);
end;

procedure TLYV_Lastborrow.BDT1Click(Sender: TObject);
begin
  DelDet.First;
end;

procedure TLYV_Lastborrow.BDT2Click(Sender: TObject);
begin
  DelDet.prior;
end;

procedure TLYV_Lastborrow.BDT3Click(Sender: TObject);
begin
  DelDet.next;
end;

procedure TLYV_Lastborrow.BDT4Click(Sender: TObject);
begin
  DelDet.last;
end;

procedure TLYV_Lastborrow.BDT5Click(Sender: TObject);
begin
  ...
  begin
    LYV_Lastborrow_PrintA4:=TLYV_Lastborrow_PrintA4.create(nil);
    LYV_Lastborrow_PrintA4.quickrep1.prepare;
    LYV_Lastborrow_PrintA4.Qpage1.caption:=inttostr(LYV_Lastborrow_PrintA4.quickrep1.QRPrinter.pagecount);
    LYV_Lastborrow_PrintA4.quickrep1.preview;
    LYV_Lastborrow_PrintA4.free;
  end;
...
procedure TLYV_Lastborrow.DBGridh3DblClick(Sender: TObject);
begin
...
procedure TLYV_Lastborrow.FormDestroy(Sender: TObject);
begin
   LYV_LASTBORROW:=nil;
end;

procedure TLYV_Lastborrow.btnBE1Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TLYV_Lastborrow.btnBE5Click(Sender: TObject);
begin
  with KCLLSS do
  begin
    active:=false;
...
procedure TLYV_Lastborrow.btnBE2Click(Sender: TObject);
begin
...
procedure TLYV_Lastborrow.btnBE3Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TLYV_Lastborrow.btnBE4Click(Sender: TObject);
var i:integer;
    Qty:real;
begin
            ...
            usinserted:     //新增狀態
              begin
                ...
                begin
                    ...
                    KCLLSS.fieldbyname('ZLBH').Value:='LAST';
...
procedure TLYV_Lastborrow.pgcPC1Changing(Sender: TObject;
...
procedure TLYV_Lastborrow.DBGridh3GetCellParams(Sender: TObject;
...
procedure TLYV_Lastborrow.dbgrdh4GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  if DelDet.FieldByName('YN').value='0' then
...
procedure TLYV_Lastborrow.DBGrid6Columns2NotInList(Sender: TObject;
...
procedure TLYV_Lastborrow.DBGrid6Columns3NotInList(Sender: TObject;
  NewText: String; var RecheckInList: Boolean);
begin
...
procedure TLYV_Lastborrow.DBGrid6Columns4NotInList(Sender: TObject;
  NewText: String; var RecheckInList: Boolean);
begin
   showmessage('Pls select Size from list ,can not alter by manually.');
   abort;
...
procedure TLYV_Lastborrow.dbgrdh6Columns0NotInList(Sender: TObject;
  NewText: String; var RecheckInList: Boolean);
begin
  showmessage('Pls select Size from list ,can not alter by manually.');
  abort;
...
procedure TLYV_Lastborrow.BDT7Click(Sender: TObject);
begin
  ...
  begin
    LYV_Lastborrow_S:=TLYV_Lastborrow_S.create(nil);
    LYV_Lastborrow_S.quickrep1.prepare;
    LYV_Lastborrow_S.Qpage1.caption:=inttostr(LYV_Lastborrow_S.quickrep1.QRPrinter.pagecount);
    LYV_Lastborrow_S.quickrep1.preview;
    LYV_Lastborrow_S.free;
  end;
  with DelMas do
  begin
    requestlive:=true;
...
procedure TLYV_Lastborrow.dbgrdh6GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
