...
uses
  ...
  DBGrids,dateutils, GridsEh, DBGridEh,comobj, PrnDbgeh,IniFiles,StrUtils;
...
type
  TDeliverCon = class(TForm)
    ...
    DBGridEh4: TDBGridEh;
    Splitter6: TSplitter;
    KCCLMONTH_BDepQry: TQuery;
    DS7: TDataSource;
    KCCLMONTH_BDepQryCKBH: TStringField;
    KCCLMONTH_BDepQryCLBH: TStringField;
    KCCLMONTH_BDepQryDepID: TStringField;
    KCCLMONTH_BDepQryQty: TFloatField;
    KCCLMONTH_BDepQryDepName: TStringField;
    KCCLMONTH_BDepQryDepMemo: TStringField;
    CKKCCLMONTH_Dep: TCheckBox;
    DelDetDepID: TStringField;
    TongVPanel: TPanel;
    DBGridEh5: TDBGridEh;
    Splitter7: TSplitter;
    DBGridEh6: TDBGridEh;
    KCCLMONTH_BDepNotQry: TQuery;
    ...
    DS8: TDataSource;
    DelDetNK1: TCurrencyField;
    DelDetTC1: TCurrencyField;
    DelDetHD1: TCurrencyField;
    DelDetKD1: TCurrencyField;
    DelDetH11: TCurrencyField;
    DelDetCWBH: TStringField;
    DelDetHGBH1: TStringField;
    DelDetHGBH_HD1: TStringField;
    DelDetHGBH_TC1: TStringField;
    DelDetNQ: TCurrencyField;
    DelDetHGBH_NQ: TStringField;
    DelDetNKNQ: TCurrencyField;
    DelDetHGBH_NKNQ: TStringField;
    DelDetPKNO: TStringField;
    DelDetDeclaretion: TStringField;
    Query1: TQuery;
    DelDetHGLB1: TStringField;
    DelDetFlag: TStringField;
    DelDetCLBH_ExcessMaterial: TStringField;
    ...
    procedure CKKCCLMONTH_DepClick(Sender: TObject);
...
uses main1, MaterialArea1, FunUnit, PKNOConfirm1;
...
begin
  ...
  begin
    try
      ...
      CheckGSBH_XT:=MyIni.ReadString('ERP','Warehouse_N211_CheckGSBH_XT','CDC');
    finally
      MyIni.Free;
    end;
  end;
  ...
  TCurrencyField(DelDet.FieldByName('NK1')).DisplayFormat:=WH_DiplayFormat;
  TCurrencyField(DelDet.FieldByName('HD1')).DisplayFormat:=WH_DiplayFormat;
  TCurrencyField(DelDet.FieldByName('KD1')).DisplayFormat:=WH_DiplayFormat;
  TCurrencyField(DelDet.FieldByName('TC1')).DisplayFormat:=WH_DiplayFormat;
  TCurrencyField(DelDet.FieldByName('H11')).DisplayFormat:=WH_DiplayFormat;
  TCurrencyField(DelDet.FieldByName('NQ')).DisplayFormat:=WH_DiplayFormat;
  TCurrencyField(DelDet.FieldByName('NKNQ')).DisplayFormat:=WH_DiplayFormat;
  ...
  begin
    DBGrid2.Columns[24].Visible:=false;
    ...
    HGLBCB.Items.Add('KD');
    HGLBCB.Items.Add('GC');
    HGLBCB.Items.Add('NK1');
    HGLBCB.Items.Add('TC1');
    HGLBCB.Items.Add('HD1');
    HGLBCB.Items.Add('KD1');
    HGLBCB.Items.Add('H11');
    HGLBCB.Items.Add('NQ');
    HGLBCB.Items.Add('NKNQ');
    HGLBCB.ItemIndex:=0;

    DBGrid2.Columns[14].PickList.Clear;
    DBGrid2.Columns[14].PickList.Add('NK');
    DBGrid2.Columns[14].PickList.Add('TC');
    DBGrid2.Columns[14].PickList.Add('HD');
    DBGrid2.Columns[14].PickList.Add('KD');
    DBGrid2.Columns[14].PickList.Add('GC');
    DBGrid2.Columns[14].PickList.Add('NK1');
    DBGrid2.Columns[14].PickList.Add('TC1');
    DBGrid2.Columns[14].PickList.Add('HD1');
    DBGrid2.Columns[14].PickList.Add('KD1');
    DBGrid2.Columns[14].PickList.Add('H11');
    DBGrid2.Columns[14].PickList.Add('NQ');
    DBGrid2.Columns[14].PickList.Add('NKNQ');
  end;
end;
...
//20230330總務倉組別檢查
procedure TDeliverCon.Init_KCCLMONTH_BDep();
begin
  ...
  begin
    ...
    begin
      ...
      SQL.Add('Update KCCLDAY_BDepartment Set Qty=0 where Qty<0 ');   //20230720 update qty<0
      ExecSQL();
      //funcObj.WriteErrorLog(sql.Text);
      Active:=false;
    end;
  end;
  with KCCLMONTH_BDepQry do
  begin
    Active:=false;
    SQL.Clear;
    SQL.Add('Select KCCLDAY_BDepartment.*,IsNull(Bdepartment.DepName,''Stock'') as DepName,IsNull(Bdepartment.DepMemo,''庫存'') as DepMemo  ');
    SQL.Add('from KCCLDAY_BDepartment ');
    SQL.Add('Left join BDepartment on BDepartment.ID=KCCLDAY_BDepartment.DepID ');
    SQL.Add('where  KCCLDAY_BDepartment.Qty>0 and KCCLDAY_BDepartment.CLBH=:CLBH and KCCLDAY_BDepartment.CKBH=:CKBH and (KCCLDAY_BDepartment.DepID=:DepID or KCCLDAY_BDepartment.DepID=''ZZZZZZZZZZ'')  ');
  end;
  with KCCLMONTH_BDepNotQry do
  begin
    Active:=false;
    SQL.Clear;
    SQL.Add('Select KCCLDAY_BDepartment.*,IsNull(Bdepartment.DepName,''Stock'') as DepName,IsNull(Bdepartment.DepMemo,''庫存'') as DepMemo  ');
    SQL.Add('from KCCLDAY_BDepartment ');
    SQL.Add('Left join BDepartment on BDepartment.ID=KCCLDAY_BDepartment.DepID ');
    SQL.Add('where  KCCLDAY_BDepartment.Qty>0 and  KCCLDAY_BDepartment.CLBH=:CLBH and KCCLDAY_BDepartment.CKBH=:CKBH and (KCCLDAY_BDepartment.DepID<>:DepID and KCCLDAY_BDepartment.DepID<>''ZZZZZZZZZZ'')  ');
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      else if DelDet.FieldByName('NK1').Value>0 then
         DelDet.FieldByName('HGLB').Value:='NK1'
      else if DelDet.FieldByName('TC').Value>0 then
         DelDet.FieldByName('HGLB').Value:='TC'
      else if DelDet.FieldByName('TC1').Value>0 then
         DelDet.FieldByName('HGLB').Value:='TC1'
      else if DelDet.FieldByName('NQ').Value>0 then
         DelDet.FieldByName('HGLB').Value:='NQ'
      else if DelDet.FieldByName('NKNQ').Value>0 then
         DelDet.FieldByName('HGLB').Value:='NKNQ'
      else if DelDet.FieldByName('HD').Value>0 then
         DelDet.FieldByName('HGLB').Value:='HD'
      else if DelDet.FieldByName('HD1').Value>0 then
         DelDet.FieldByName('HGLB').Value:='HD1'
      ...
      else if DelDet.FieldByName('KD1').Value>0 then
         DelDet.FieldByName('HGLB').Value:='KD1'
      {else
         DelDet.FieldByName('HGLB').Value:='XT'; }
      else
        DelDet.FieldByName('HGLB').Value:=NULL;
      DelDet.Post;
    end;
  ...
  end else if (DelDet.FieldByName('HGLB').Value='NK1')  then
  begin
    if  DelDet.FieldByName('CNO').Value=null then
      DelDet.FieldByName('CNO').Value:=DelDet.FieldByName('HGBH1').Value
  end else if  (DelDet.FieldByName('HGLB').Value='TC') then
  begin
    if  DelDet.FieldByName('CNO').Value=null then
      DelDet.FieldByName('CNO').Value:=DelDet.FieldByName('HGBH_TC').Value
  end else if  (DelDet.FieldByName('HGLB').Value='TC1') then
  begin
    if  DelDet.FieldByName('CNO').Value=null then
      DelDet.FieldByName('CNO').Value:=DelDet.FieldByName('HGBH_TC1').Value
  end else if  (DelDet.FieldByName('HGLB').Value='NQ') then
  begin
    if  DelDet.FieldByName('CNO').Value=null then
      DelDet.FieldByName('CNO').Value:=DelDet.FieldByName('HGBH_NQ').Value
  end else if (DelDet.FieldByName('HGLB').Value='NKNQ') then
  begin
    if  DelDet.FieldByName('CNO').Value=null then
      DelDet.FieldByName('CNO').Value:=DelDet.FieldByName('HGBH_NKNQ').Value
  end else if (DelDet.FieldByName('HGLB').Value='HD')  then
  begin
    if  DelDet.FieldByName('CNO').Value=null then
      DelDet.FieldByName('CNO').Value:=DelDet.FieldByName('HGBH_HD').Value
  end else if  (DelDet.FieldByName('HGLB').Value='HD1')  then
  begin
    if  DelDet.FieldByName('CNO').Value=null then
      DelDet.FieldByName('CNO').Value:=DelDet.FieldByName('HGBH_HD1').Value
  end else if ((DelDet.FieldByName('HGLB').Value='KD') or (DelDet.FieldByName('HGLB').Value='KD1')) then
  begin
  ...
  end else if DelDet.FieldByName('HGLB').AsString='NK1' then
  begin
    if  DelDet.FieldByName('Qty').Value>DelDet.FieldByName('NK1').Value then iRes:=false;
  ...
  end else if DelDet.FieldByName('HGLB').AsString='TC1' then
  begin
    if  DelDet.FieldByName('Qty').Value>DelDet.FieldByName('TC1').Value then iRes:=false;
  end else if DelDet.FieldByName('HGLB').AsString='NQ' then
  begin
    if  DelDet.FieldByName('Qty').Value>DelDet.FieldByName('NQ').Value then iRes:=false;
  end else if DelDet.FieldByName('HGLB').AsString='NKNQ' then
  begin
    if  DelDet.FieldByName('Qty').Value>DelDet.FieldByName('NKNQ').Value then iRes:=false;
  ...
  end else if DelDet.FieldByName('HGLB').AsString='HD1' then
  begin
    if  DelDet.FieldByName('Qty').Value>DelDet.FieldByName('HD1').Value then iRes:=false;
  ...
  end else if DelDet.FieldByName('HGLB').AsString='KD1' then
  begin
    if  DelDet.FieldByName('Qty').Value>DelDet.FieldByName('KD1').Value then iRes:=false;
  ...
  end else if DelDet.FieldByName('HGLB').AsString='H11' then
  begin
    if  DelDet.FieldByName('Qty').Value>DelDet.FieldByName('H11').Value then iRes:=false;
  end else
  begin
    iRes:=false;
  end;
  ...
  begin
    ...
    begin
      if ((DelDet.FieldByName('HGLB').AsString='XT') or (DelDet.FieldByName('HGLB').AsString='H11')) then
      begin
      ...
      if (((DelDet.FieldByName('HGLB').AsString='KD') or (DelDet.FieldByName('HGLB').AsString='KD1')) and (DelDet.FieldByName('CNO').AsString='')) then
      begin
        Info:=DelDet.FieldByName('HGLB').AsString+' khong cho xuat kho, vi don hang xuat nuoc ngoai. 出口訂單不允許使用';
        iRes:=false;
      end;
    ...
    begin
      if ((DelDet.FieldByName('HGLB').AsString='NK') or (DelDet.FieldByName('HGLB').AsString='NK1')) then
      begin
        Info:=DelDet.FieldByName('HGLB').AsString+' khong cho xuat kho, vi su dung xuong .內銷&廠內不允許使用';
        iRes:=false;
      end;
      if ((DelDet.FieldByName('HGLB').AsString='TC') or (DelDet.FieldByName('HGLB').AsString='TC1') or (DelDet.FieldByName('HGLB').AsString='NQ') or (DelDet.FieldByName('HGLB').AsString='NKNQ') ) then
      begin
  ...
  {if DelDet.FieldByName('Qty').Value=0 then iRes:=true;  }//20231016 check Qty=0, must key in HGLB & CNO
  if ((DelDet.FieldByName('Qty').Value=0) and (DelDet.FieldByName('CWBH').AsString<>'153'))  then
  begin
    if  (DelDet.FieldByName('HGLB').AsString ='') or (DelDet.FieldByName('HGLB').AsString='XT') then
    begin
      showmessage('Pls choose HGLB');
      abort;
    end else
    begin
      if ((DelDet.FieldByName('HGLB').AsString='NK') or (DelDet.FieldByName('HGLB').AsString='TC') or (DelDet.FieldByName('HGLB').AsString='HD') or (DelDet.FieldByName('HGLB').AsString='NK1')
          or (DelDet.FieldByName('HGLB').AsString='TC1') or (DelDet.FieldByName('HGLB').AsString='NQ') or (DelDet.FieldByName('HGLB').AsString='NKNQ')  or (DelDet.FieldByName('HGLB').AsString='HD1') ) and (DelDet.FieldByName('CNO').AsString='')  then
      begin
        showmessage('Pls choose CNO');
        abort;
      end else
      begin
        Info:='OK';
  ...
  //20231108 can not delivery XT with SCBH = JJJJJJJJJJ
  if ((DelDet.FieldByName('Qty').Value>0)  and (DelDet.FieldByName('HGLB').AsString='XT')) then
  begin
    if (DelMas.FieldByName('SCBH').AsString='JJJJJJJJJJ') then
    begin
      if ((copy(DelDet.FieldByName('SCBH').AsString,1,4)<>'LAIT') and (copy(DelDet.FieldByName('SCBH').AsString,1,4)<>'LAIK') and (copy(DelDet.FieldByName('SCBH').AsString,1,4)<>'CBYK')) then
      begin
        showmessage('Do not allow delivery XT');
        abort;
      end;
    end;
  end;
  //
  //Chon to khai phieu xuat
  if ((RightStr(DelMas.FieldByName('CKBH').AsString,1) <>'X') and (RightStr(DelMas.FieldByName('CKBH').AsString,1) <>'Y') and (RightStr(DelMas.FieldByName('CKBH').AsString,1) <>'Z')) or (DelMas.FieldByName('CKBH').AsString = 'CBY') then
  begin
    if ((DelDet.FieldByName('SCBH').AsString='ZZZZZZZZZZ') and (DelDet.FieldByName('Qty').value>0) and (copy(DelDet.FieldByName('HGLB').AsString,1,2)='NK') and (DelDet.FieldByName('Flag').AsString<>'1') ) then
    begin
      if DelDet.FieldByName('Declaretion').AsString='' then
      begin
        showmessage('Chua chon to khai');
        abort;
      end else
      begin
         if ((DelDet.FieldByName('HGLB1').AsString <> '')  and (DelDet.FieldByName('HGLB').AsString <> DelDet.FieldByName('HGLB1').AsString)) then
         begin
            showmessage('Vui long chon lai chon to khai');
            abort;
         end;
      end;
    end;
  end;
  //Kiem tra vat tu kho CBY qua mua da bao so luong ok thi khong the xuat phieu
  if ((DelMas.FieldByName('CKBH').AsString='CBY') and (DelDet.FieldByName('CLBH_ExcessMaterial').AsString<>'')) then
  begin
    ShowMessage('Ma vat tu '+DelDet.FieldByName('CLBH').AsString+' qua mua da bao so luong ok khong the xuat phieu.');
    Abort;
  end;
    //
  result:=iRes;
...
begin
  ...
  if WH_TXSQL_Force_Check='Y' then
  begin
    if Pos(DelMas.FieldByName('CKBH').AsString,WH_TXSQL)>0 then
    begin
      CKKCCLMONTH_Dep.Checked:=true;
    end else
    begin
      CKKCCLMONTH_Dep.Checked:=false;
    end;
  end;
  //暫存檢查庫存同一筆領料單多筆料號
  ...
  begin
    ...
    begin
        ...
        //20230328 檢查總務領料組別
        if CKKCCLMONTH_Dep.Checked=true then
        begin
          if KCCLMONTH_BDepQry.Active=false then KCCLMONTH_BDepQry.Active:=true;
          if KCCLMONTH_BDepNotQry.Active=false then KCCLMONTH_BDepNotQry.Active:=true;
          if (DelDet.FieldByName('Qty').Value> DBGridEh5.getfootervalue(0,DBGridEh5.columns[3])) then
          begin
            showmessage('Don vi Ton Kho So luong khong du.組別單位 庫存數量不夠發');
            abort();
          end;
        end;
        ...
        begin
          ...
          begin
            Showmessage('Qty VS SiZE Qty is not the same.尺碼數量和總領料數量不一致');
            abort();
          end;
        end;
        DelDet.next;
    ...
    try  //更新真正庫存
      ...
      begin
          ...
          begin
              ...
              begin
                ...
                //20240829 luu so phieu nhap kho vo bang KCPKLL
                ...
                begin
                  ...
                  begin
                    active:=false;
                    sql.Clear;
                    sql.Add('Delete KCPKLL');
                    sql.Add('Where LLNO='''+DelDet.fieldbyname('LLNO').AsString+''' and CLBH='''+DelDet.fieldbyname('CLBH').AsString+''' and DFL='''+DelDet.fieldbyname('DFL').AsString+''' and SCBH='''+DelDet.fieldbyname('SCBH').AsString+''' and YN=2');
                    sql.Add('Insert into KCPKLL (LLNO,CLBH,DFL,SCBH,PKNO,Qty,UserID,UserDate,YN)');
                    sql.Add('values ('''+DelDet.fieldbyname('LLNO').AsString+''', '''+DelDet.fieldbyname('CLBH').AsString+''', '''+DelDet.fieldbyname('DFL').AsString+''','''+DelDet.fieldbyname('SCBH').AsString+''' ');
                    sql.Add('        ,'''+DelDet.fieldbyname('PKNO').AsString+''','''+formatfloat(WH_DiplayFormat,DelDet.fieldbyname('Qty').Value)+''','''+ main.edit1.text+''',GetDate(),''2'') ');
                    //funcobj.WriteErrorLog(sql.Text);
                    Execsql;
                  end;
                end;
      ...
      begin
        ...
        SQL.Add('  NK1 = case when KCLLS.HGLB=''NK1'' then NK1-KCLLS.Qty else NK1 end,');
        SQL.Add('  TC1 = case when KCLLS.HGLB=''TC1'' then TC1-KCLLS.Qty else TC1 end,');
        SQL.Add('  HD1 = case when KCLLS.HGLB=''HD1'' then HD1-KCLLS.Qty else HD1 end,');
        SQL.Add('  KD1 = case when KCLLS.HGLB=''KD1'' then KD1-KCLLS.Qty else KD1 end,');
        SQL.Add('  H11 = case when KCLLS.HGLB=''H11'' then H11-KCLLS.Qty else H11 end,');
        SQL.Add('  NQ = case when KCLLS.HGLB=''NQ'' then NQ-KCLLS.Qty else NQ end,');
        SQL.Add('  NKNQ = case when KCLLS.HGLB=''NKNQ'' then NKNQ-KCLLS.Qty else NKNQ end');
        SQL.Add('from (');
      ...
      begin
        ...
        begin
          Active:=false;
          SQL.Clear;
          SQL.Add('Update  KCCLDAY_BDepartment Set Qty=KCCLDAY_BDepartment.Qty-A.Qty');
          SQL.Add('from (');
          SQL.Add('Select A.CKBH,Case when (IsNull(KCCLDAY_BDepartment.Qty,0.0)-A.Qty)<0 then ''ZZZZZZZZZZ'' else A.DepID end as DepID,A.CLBH, A.Qty from (');
          SQL.Add('Select KCLL.CKBH,KCLL.DepID,KCLLS.CLBH,Sum(KCLLS.Qty) as Qty');
          SQL.Add('from KCLLS');
          SQL.Add('left join KCLL on KCLL.LLNO=KCLLS.LLNO');
          SQL.Add('where KCLL.LLNO='''+DelMas.FieldByName('LLNO').AsString+'''');
          SQL.Add('Group by KCLL.CKBH,KCLL.DepID,KCLLS.CLBH  )  A');
          SQL.Add('left join KCCLDAY_BDepartment on KCCLDAY_BDepartment.CKBH=A.CKBH and KCCLDAY_BDepartment.CLBH=A.CLBH and KCCLDAY_BDepartment.DepID=A.DepID   ');
          SQL.Add(')   A');
          SQL.Add('where KCCLDAY_BDepartment.CKBH=A.CKBH and KCCLDAY_BDepartment.CLBH=A.CLBH and KCCLDAY_BDepartment.DepID=A.DepID   ');
          Execsql;
        end;
...
begin
 ...
 try  
  ...
  begin
      ...
      sql.Add('    ,NK=null , TC=null , HD=null , KD=null, GC=null, XT=null, NK1=null, TC1=null, HD1=null, KD1=null, H11=null, NQ=null, NKNQ=null ');
      sql.add('where  KCZLS.CKBH in (select CKBH from KCCK where GSBH='''+main.Edit2.Text+''')   ');
  ...
  begin
    Active:=false;
    Sql.Clear;
    SQL.Add('Update KCZLS set NK=HG.NK,TC=HG.TC,HD=HG.HD,KD=HG.KD,GC=HG.GC,XT=HG.XT,ZZZZ=HG.ZZZZ,NK1=HG.NK1,TC1=HG.TC1,HD1=HG.HD1,KD1=HG.KD1,H11=HG.H11,NQ=HG.NQ,NKNQ=HG.NKNQ');
    SQL.Add('from (');
    SQL.Add('select CKBH,CLBH,Max(NK) as NK,Max(TC) as TC,Max(HD) as HD,Max(KD) as KD,Max(GC) as GC,Max(XT) as XT,Max(ZZZZ) as ZZZZ ');
    SQL.Add('                ,Max(NK1) as NK1,Max(TC1) as TC1,Max(HD1) as HD1,Max(KD1) as KD1, Max(H11) as H11,Max(NQ) as  NQ,Max(NKNQ) as NKNQ from (');
    ...
    SQL.Add('       case when HGLB=''NK1'' then Max(Qty)  end as NK1,');
    SQL.Add('       case when HGLB=''TC1'' then Max(Qty)  end as TC1,');
    SQL.Add('       case when HGLB=''HD1'' then Max(Qty)  end as HD1,');
    SQL.Add('       case when HGLB=''KD1'' then Max(Qty)  end as KD1,');
    SQL.Add('       case when HGLB=''H11'' then Max(Qty)  end as H11,');
    SQL.Add('       case when HGLB=''NQ'' then Max(Qty)  end as NQ,');
    SQL.Add('       case when HGLB=''NKNQ'' then Max(Qty) end as NKNQ');
    SQL.Add('from (');
    ...
    SQL.Add('   union all select  ''NK1'' union all select ''TC1'' union all select ''HD1'' union all select ''KD1'' union all select ''H11'' union all select ''NQ'' union all select ''NKNQ'') LOAI ) KCZLS');
...
procedure TDeliverCon.CKKCCLMONTH_DepClick(Sender: TObject);
begin
  //檢查總務組別領料
  if CKKCCLMONTH_Dep.Checked=true then
  begin
    TongVPanel.Visible:=true;
    KCCLMONTH_BDepQry.Active:=true;
    Splitter5.Align:=alleft;
  end else
  begin
    TongVPanel.Visible:=false;
    KCCLMONTH_BDepQry.Active:=false;
    Splitter5.Align:=alNone;
  end;
end;

procedure TDeliverCon.DBGrid2EditButtonClick(Sender: TObject);
begin
  if DBGrid2.SelectedField.FieldName='Declaretion'  then
  begin
    PKNOConfirm:=TPKNOConfirm.create(self);
    PKNOConfirm.Edit1.Text:=DelDet.fieldbyname('CLBH').Value;
    PKNOConfirm.Edit2.Text:=DelDet.fieldbyname('HGLB').Value;
    PKNOConfirm.Edit5.Text:=DelMas.fieldbyname('CKBH').Value;
    PKNOConfirm.show;
  end;
end;
...
begin
   DBGrid2.columns[30].ButtonStyle:=cbsEllipsis;
end;
