...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.add('select OSRK.*,BDepartment.DepName,LastKCCK.Memo as CKBH_Memo,Case when OSRK.SFL=''1'' then ''TraÂkÁÙ'' when OSRK.SFL=''2'' then ''Mua moi¤J®w'' end as SFL_DESC');
    sql.add('from OSRK');
    sql.add('left join BDepartment on OSRK.DepID=BDepartment.ID');
    sql.add('Left join LastKCCK on OSRK.CKBH=LastKCCK.CKBH ');
    sql.add('where OSRK.SB=''L''  and isnull(OSRK.flowflag,'''') <>''X''  ');
    IF Edt1.TEXT<>'' THEN
...
function TOutsolereturn.CheckIfLLNO(LLNO:string):boolean;
begin
   with Query1 do
   begin
     Active:=false;
     SQL.CLear;
     SQL.Add('Select LLNO from OSLL where LLNO='''+LLNO+''' ');
     Active:=true;
     if RecordCount>0 then
       Result:=true
     else
...
procedure TOutsolereturn.LTRKSS_ZLBH_Value();
begin
...
procedure TOutsolereturn.btnBE6Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
