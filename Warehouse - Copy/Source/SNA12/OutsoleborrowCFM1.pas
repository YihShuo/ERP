...
type
  TOutsoleborrowCFM = class(TForm)
    ...
    btn4: TButton;
    qryDelMasLLNO: TStringField;
    qryDelMasGSBH: TStringField;
    qryDelMasCKBH: TStringField;
    qryDelMasPlanDate: TDateTimeField;
    qryDelMasDepID: TStringField;
    qryDelMasUSERDATE: TDateTimeField;
    qryDelMasUSERID: TStringField;
    qryDelMasCFMDate: TDateTimeField;
    qryDelMasCFMID: TStringField;
    qryDelMasSB: TStringField;
    qryDelMasYN: TStringField;
    qryDelMasPMARK: TIntegerField;
    qryDelMasMEMO: TStringField;
    qryDelMasSFL: TStringField;
    qryDelMasDepName: TStringField;
...
begin
  ...
  begin
    ...
    SQL.Add('   where OSLLSS.LLNO=OSLL.LLNO  and OSLL.CFMID<>''NO'' and OSLL.GSBH='''+main.Edit2.Text+''' and isnull(OSLL.flowflag,'''') <>''X'' ');
    SQL.Add('   and convert(smalldatetime,convert(varchar,OSLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
...
begin
  ...
  begin
    Active:=false;
    sql.Clear;
    sql.add('select OSLL.*,BDepartment.DepName,Case when OSLL.SFL=''1'' then ''Muon­É¥X'' when OSLL.SFL=''2'' then ''BO PHE³ø¼o'' end as SFL_DESC  ');
    sql.add('from OSLL ');
    ...
    sql.add('and OSLL.GSBH='+''''+main.Edit2.Text+''' and isnull(OSLL.flowflag,'''') <>''X''');
  ...
  begin
    ...
    Active:=false;
    sql.Clear;
    sql.Add('select OSLLSS.LLNO,OSLLSS.CLBH,CLZL.YWPM,CLZL.DWBH,OSLLSS.ZLBH,OSLLSS.SIZ,OSLLSS.Qty,#CLZLKC_Outsole.Qty as KCQty ');
...
begin
  ...
  begin
    ...
    sql.add('where OSRKSS.RKNO=OSRK.RKNO  and OSRK.GSBH='''+main.Edit2.Text+''' and OSRKSS.ZLBH=''Outsole'' and isnull(OSRK.flowflag,'''') <>''X'' ');
    sql.add('and convert(smalldatetime,convert(varchar,OSRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
...
begin
  ...
  CalculateOutsoleStock();
  //
  if qryDelDet.RecordCount>0 then //·í¦³OSLLSS²Ä¤T¼h®É,ÀË¬dOSLLSS
  begin
...
procedure TOutsoleborrowCFM.Button4Click(Sender: TObject);
begin
  try
    with qryDelMas do
    begin
