...
begin
  ...
  begin
    ...
    SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''PHOM'' and GSBH='''+main.Edit2.Text+''' )  and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
    SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
    SQL.Add('  Union All ');
    SQL.Add('  select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH in (Select CKBH from LastKCCK where LB=''PHOM'' and GSBH='''+main.Edit2.Text+''' )  and KCYEAR='''+ayear+''' and KCMonth='''+amonth+''' ) as tmpRKTLFL ');
    SQL.Add('  group by CKBH,CLBH,SIZ ) as NowKC');
    SQL.Add('left join ( ');
    SQL.Add('   select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where CKBH in (Select CKBH from LastKCCK where LB=''PHOM'' and GSBH='''+main.Edit2.Text+''' ) and KCYEAR='''+ayear+''' and KCMonth='''+amonth+''' ) as LastKC on NowKC.CKBH=LastKC.CKBH and NowKC.CLBH=LastKC.CLBH and NowKC.SIZ=LastKC.SIZ');
    SQL.Add('left join ( ');
    SQL.Add('  select LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK ');
    SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''PHOM'' and GSBH='''+main.Edit2.Text+''' ) and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
    ...
    SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.CKBH in (Select CKBH from LastKCCK where LB=''PHOM'' and GSBH='''+main.Edit2.Text+''' ) and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
...
begin
  ...
  begin
    ...
    sql.add('where LTLL.SB=''L'' and LTLL.CFMID=''NO'' ');
...
begin
  ...
  begin
    ...
    sql.add('where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.CKBH in (Select CKBH from LastKCCK where LB=''PHOM'' and GSBH='''+main.Edit2.Text+''' ) and LTLL.SFL=''1'' and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+'''  ');
    sql.add('Group by LTLL.DepID,LTLLSS.CLBH,LTLLSS.SIZ ');
    sql.add('Union All ');
    sql.add('select DepID,CLBH,SIZ,QTY as LastRem from Lastmonth_Out where CKBH in (Select CKBH from LastKCCK where LB=''PHOM'' and GSBH='''+main.Edit2.Text+''' ) and  KCYEAR='''+ayear+''' and KCMonth='''+amonth+''') as tmpRKTLFL ');
    ...
    sql.add('where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH in (Select CKBH from LastKCCK where LB=''PHOM'' and GSBH='''+main.Edit2.Text+''' ) and LTRK.SFL=''1'' and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(Date))+''' and '''+formatdatetime('yyyy/MM/dd',Date)+''' ');
