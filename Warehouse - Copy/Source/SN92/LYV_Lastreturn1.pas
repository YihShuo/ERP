...
var
  LYV_Lastreturn: TLYV_Lastreturn;
  sdate,edate,NDate:Tdate;

implementation

uses  main1 ,LYV_LastReturn_S1,
     Lastreturn_Dep1, Lastreturn_CL1, LYV_Lastreturn_PrintA41, LastPurchase_CL1,FunUnit;
{$R *.dfm}

procedure TLYV_Lastreturn.FormClose(Sender: TObject;
...
procedure TLYV_Lastreturn.btn2Click(Sender: TObject);
var y,m:word;
begin
...
procedure TLYV_Lastreturn.BB7Click(Sender: TObject);
begin
close;
end;

procedure TLYV_Lastreturn.FormCreate(Sender: TObject);
var i:integer;
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.add('Select CKBH from LastKCCK WHERE GSBH='''+main.Edit2.text+''' order by CKBH');
    active:=true;
...
procedure TLYV_Lastreturn.DelMasAfterOpen(DataSet: TDataSet);
begin
  BB2.Enabled:=true;
  BB3.Enabled:=true;
  BB4.Enabled:=true;
...
procedure TLYV_Lastreturn.BB2Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TLYV_Lastreturn.BB3Click(Sender: TObject);
begin
...
procedure TLYV_Lastreturn.BB4Click(Sender: TObject);
begin
  with DelMas do
  begin
    requestlive:=true;
...
procedure TLYV_Lastreturn.BB5Click(Sender: TObject);
var y,m,a:string;
i:integer;
begin
               ...
               if DelMas.fieldbyname('YN').value='0'then
                 begin
                   ...
                   begin
                     ...
                     sql.add('values (''LTRK'','''+DelMas.fieldbyname('RKNO').Value+'''');
...
procedure TLYV_Lastreturn.btnBBTT1Click(Sender: TObject);
begin
  Lastreturn_Dep:=TLastreturn_Dep.Create(self);
  Lastreturn_Dep.Show;
end;

procedure TLYV_Lastreturn.BB6Click(Sender: TObject);
begin
 with DelMas do
  begin
    active:=false;
...
procedure TLYV_Lastreturn.DBGridh3EditButtonClick(Sender: TObject);
begin
 Lastreturn_Dep:=TLastreturn_Dep.Create(self);
 Lastreturn_Dep.Show;
...
procedure TLYV_Lastreturn.DelDetAfterOpen(DataSet: TDataSet);
begin
  BD2.Enabled:=true;
  BD3.Enabled:=true;
  BD4.Enabled:=true;
...
procedure TLYV_Lastreturn.BD2Click(Sender: TObject);
begin
...
procedure TLYV_Lastreturn.BD3Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
function TLYV_Lastreturn.CheckIfLLNO(LLNO:string):boolean;
begin
   with Query1 do
   begin
     Active:=false;
...
procedure TLYV_Lastreturn.LTRKSS_ZLBH_Value();
begin
  if DelMas.FieldByName('SB').AsString='L' then  //ÝÉÀY
  begin
    if DelMas.FieldByName('SFL').AsString='1' then ZLBH_Value:='LAST';
    if DelMas.FieldByName('SFL').AsString='2' then ZLBH_Value:='NEWL';
  end;
  if DelMas.FieldByName('SB').AsString='C' then  //±Ù¤M
  begin
    if DelMas.FieldByName('SFL').AsString='1' then ZLBH_Value:='DAOC';
    if DelMas.FieldByName('SFL').AsString='2' then ZLBH_Value:='NEWC';
  end;
end;
procedure TLYV_Lastreturn.BD5Click(Sender: TObject);
var i:integer;
begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      sql.add(' where LastSizeR.CLBH='+''''+DelDet.fieldbyname('CLBH').AsString+'''');
                      execsql;
                      active:=false;
                      close;
                    end;
...
procedure TLYV_Lastreturn.BD4Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TLYV_Lastreturn.BD6Click(Sender: TObject);
begin
  with DelDet do
  begin
    active:=false;
...
procedure TLYV_Lastreturn.btnBBTT2Click(Sender: TObject);
begin

  if messagedlg('Phom giay tra ve hay hoac muc hang nhap kho ?',mtinformation,[mbYes,mbNo],0)=mrYes then
  begin
    Lastreturn_CL:=TLastreturn_CL.create(self);
    Lastreturn_CL.DepIDEdit.Text:=DelMas.FieldByName('DepID').AsString;
    Lastreturn_CL.show;
  end else
  begin
    LastPurchase_CL:=TLastPurchase_CL.create(self);
    LastPurchase_CL.show;
  end;

end;

procedure TLYV_Lastreturn.dbgrdh4EditButtonClick(Sender: TObject);
begin
btnBBTT2Click(nil);
end;

procedure TLYV_Lastreturn.BDT5Click(Sender: TObject);
begin
  ...
  LYV_Lastreturn_PrintA4:=TLYV_Lastreturn_PrintA4.create(nil);
  LYV_Lastreturn_PrintA4.quickrep1.prepare;
  LYV_Lastreturn_PrintA4.Qpage1.caption:=inttostr(LYV_Lastreturn_PrintA4.quickrep1.QRPrinter.pagecount);
  LYV_Lastreturn_PrintA4.quickrep1.preview;
  LYV_Lastreturn_PrintA4.free;
...
procedure TLYV_Lastreturn.FormDestroy(Sender: TObject);
begin
   LYV_Lastreturn:=nil;
end;

procedure TLYV_Lastreturn.btnBE1Click(Sender: TObject);
begin
...
procedure TLYV_Lastreturn.btnBE5Click(Sender: TObject);
begin
...
procedure TLYV_Lastreturn.btnBE2Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TLYV_Lastreturn.btnBE3Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
...
procedure TLYV_Lastreturn.btnBE4Click(Sender: TObject);
var i:integer;
    Qty:real;
begin
  LTRKSS_ZLBH_Value();//§ó·sÀx¦sLTLLSS.ZLBH¬O·s±ÄÁÊÁÙ¬OÂkÁÙ
    ...
    for i:=1 to KCLLSS.RecordCount do
      ...
      with query1 do       //„h³ý²»´æÔÚµÄí—
        begin
          ...
          sql.add('                             and LTRKSS.RKNo='+''''+DelDet.FieldByName('RKNo').Value+''''+' and LTRKSS.CLBH='+''''+DelDet.FieldByName('CLBH').Value+'''' );
          sql.add('                             GROUP BY RKNo,CLBH),0) ');
          sql.add(' where LTRKS.RKNo='+''''+DelDet.FieldByName('RKNo').Value+''''+ ' and LTRKS.CLBH='+''''+DelDet.FieldByName('CLBH').Value+'''' );
          execsql;
          active:=false;
          sql.Clear;
          sql.add('UPDATE LTLLS set RKQTY=isnull((select isnull(SUM(QTY),0) AS SQTY from LTRKSS   ');
          sql.add('                             where LTRKSS.ZLBH='+''''+DelDet.FieldByName('ZLBH').Value+''''+' and LTRKSS.CLBH='+''''+DelDet.FieldByName('CLBH').Value+'''' );
          sql.add('                             GROUP BY RKNo,CLBH),0) ');
          sql.add(' where LTLLS.LLNo='+''''+DelDet.FieldByName('ZLBH').Value+''''+ ' and LTLLS.CLBH='+''''+DelDet.FieldByName('CLBH').Value+'''' );
          execsql;
          close;
        end;
      DelDet.Active:=false;
...
procedure TLYV_Lastreturn.pgcPC1Changing(Sender: TObject;
...
procedure TLYV_Lastreturn.DBGridh3GetCellParams(Sender: TObject;
...
procedure TLYV_Lastreturn.dbgrdh4GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
  if DelDet.FieldByName('YN').value='0' then
...
procedure TLYV_Lastreturn.dbgrdh6GetCellParams(Sender: TObject;
  Column: TColumnEh; AFont: TFont; var Background: TColor;
  State: TGridDrawState);
begin
...
procedure TLYV_Lastreturn.DBGrid6Columns2NotInList(Sender: TObject;
...
procedure TLYV_Lastreturn.DBGrid6Columns3NotInList(Sender: TObject;
  NewText: String; var RecheckInList: Boolean);
begin
...
procedure TLYV_Lastreturn.DBGrid6Columns4NotInList(Sender: TObject;
  NewText: String; var RecheckInList: Boolean);
begin
  showmessage('Pls select Size from list ,can not alter by manually.');
  abort;
...
procedure TLYV_Lastreturn.KCLLSSCalcFields(DataSet: TDataSet);
begin
    KCLLSS.Active:=false;
    KCLLSS.Active:=true;
...
procedure TLYV_Lastreturn.dbgrdh6Columns1NotInList(Sender: TObject;
  NewText: String; var RecheckInList: Boolean);
begin
  showmessage('Pls select Size from list ,can not alter by manually.');
  abort;
...
procedure TLYV_Lastreturn.BDT7Click(Sender: TObject);
begin
  if DelDet.RequestLive then
  begin
    showmessage('Pls save data first.');
  ...
  begin
    LYV_LastReturn_S:=TLYV_LastReturn_S.create(nil);
    LYV_LastReturn_S.quickrep1.prepare;
    LYV_LastReturn_S.Qpage1.caption:=inttostr(LYV_LastReturn_S.quickrep1.QRPrinter.pagecount);
    LYV_LastReturn_S.quickrep1.preview;
    LYV_LastReturn_S.free;
  end;
  with DelMas do
  begin
    requestlive:=true;
...
procedure TLYV_Lastreturn.PGCPC1Change(Sender: TObject);
begin
...
procedure TLYV_Lastreturn.BB1Click(Sender: TObject);
begin
  MasToolPanel.Visible:=true;
end;

procedure TLYV_Lastreturn.DBGridh3DblClick(Sender: TObject);
begin
...
procedure TLYV_Lastreturn.btnBE6Click(Sender: TObject);
begin
  if DelMas.RequestLive then
  begin
    showmessage('Pls save Master data first.');
