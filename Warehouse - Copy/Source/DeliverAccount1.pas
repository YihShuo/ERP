...
begin
  ...
  begin
    ...
    begin
      ...
      sql.add('             left join  ');  
      if OldCK.Checked=true  then
        sql.add('          (select * from kcll union all select * from kcll_2014) KCLL')
      else
        sql.add('           KCLL ');
      sql.Add('             on KCLL.LLNO=KCLLS.LLNO');
      sql.add('           left join DDZL on DDZL.DDBH=KCLLS.SCBH');
      sql.add('           left join CLZL on CLZL.CLDH=KCLLS.CLBH');
      sql.Add('           where CLBH like '+''''+edit1.Text+'%'+'''');
      sql.add('           and KCLLS.SCBH like '+''''+edit4.Text+'%'+'''');
    ...
    begin
      ...
      begin
         sql.add('   select DDZL.BuyNo,ZLZLS2.ZLBH,ZLZLS2.MJBH,(select top 1 Case when  A.MJBH<>''ZZZZZZZZZZ'' then A.MJBH else ZLZLS2.MJBH end as MJBH  ');
         if OldCK.Checked=true then
            sql.add('    from (select * from zlzls2 union all select * from zlzls2_2014)A')
         else
            sql.add('    from '+ZLZLS2_TB+' A');
         sql.Add('  where A.CLBH=ZLZLS2.MJBH and A.ZLBH=ZLZLS2.ZLBH  ) as Parent,''Gia cong'' as DType,ZLZLS2.CLBH,') ;
      end else
      begin
