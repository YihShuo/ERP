...
type
  TOutsoleSTOCK = class(TForm)
    ...
    procedure dbgrdh2DrawColumnCell(Sender: TObject; const Rect: TRect;
      DataCol: Integer; Column: TColumnEh; State: TGridDrawState);
    procedure DetailRK1Click(Sender: TObject);
...
uses  main1, OutsoleSTOCK_Det1,Outsolestock_RK1, FunUnit, OutsoleStock_Detail1,
  Outsolestock_LL1, OutsoleSTOCK_PRINT1, OutsoleSTOCK_PRINT_Tong1;
...
begin
 ...
 begin
   ...
   begin
      ...
      SQL.Add('  where OSRKSS.RKNO=OSRK.RKNO  and OSRK.CKBH='''+CBX1.Text+'''  and isnull(OSRK.flowflag,'''') <>''X'' ');
      SQL.Add('  and convert(smalldatetime,convert(varchar,OSRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      ...
      SQL.Add('   where OSLLSS.LLNO=OSLL.LLNO  and OSLL.CFMID<>''NO'' and OSLL.CKBH='''+CBX1.Text+''' and isnull(OSLL.flowflag,'''') <>''X'' ');
      SQL.Add('   and convert(smalldatetime,convert(varchar,OSLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      SQL.Add('   Group by OSLL.CKBH,OSLLSS.CLBH,OSLLSS.SIZ ) LL on NowKC.CKBH=LL.CKBH and NowKC.CLBH=LL.CLBH and NowKC.SIZ=LL.SIZ');
      SQL.Add('select #CLZLKC_Outsole.CLBH,CLZL.YWPM, SUM(Qty) as Total,CLZL.DWBH,');
      ...
      SQL.Add('Group by #CLZLKC_Outsole.CLBH,CLZL.YWPM,CLZL.CQDH,CLZL.DWBH ');
 ...
 begin
   ...
   begin
      ...
      SQL.Add('select NowKC.CKBH,NowKC.CLBH,NowKC.SIZ,IsNull(NowKC.NowRKQty,0)-IsNull(LL.LLQty,0) as Qty into #CLZLKC_OutsoleK from (');
      SQL.Add('  select CKBH,CLBH,SIZ,Sum(RKQty) as NowRKQty from (');
      SQL.Add('  select OSRK.CKBH,OSRKSS.CLBH,OSRKSS.SIZ,SUM(OSRKSS.Qty) as RKQty from OSRKSS,OSRK ');
      SQL.Add('  where OSRKSS.RKNO=OSRK.RKNO  and OSRK.CKBH='''+CBX1.Text+''' and OSRKSS.ZLBH=''NEWL'' and isnull(OSRK.flowflag,'''') <>''X'' ');
      SQL.Add('  and convert(smalldatetime,convert(varchar,OSRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      SQL.Add('  Group by OSRK.CKBH,OSRKSS.CLBH,OSRKSS.SIZ ');
      SQL.Add('  Union All ');
      ...
      SQL.Add('left join ( ');
      SQL.Add('   select OSLL.CKBH,OSLLSS.CLBH,OSLLSS.SIZ,SUM(OSLLSS.Qty) as LLQty from OSLLSS,OSLL ');
      SQL.Add('   where OSLLSS.LLNO=OSLL.LLNO  and OSLL.CFMID<>''NO'' and OSLL.CKBH='''+CBX1.Text+''' and isnull(OSLL.flowflag,'''') <>''X'' and OSLL.SFL=''2'' ');
      SQL.Add('   and convert(smalldatetime,convert(varchar,OSLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      SQL.Add('   Group by OSLL.CKBH,OSLLSS.CLBH,OSLLSS.SIZ ) LL on  NowKC.CLBH=LL.CLBH and NowKC.SIZ=LL.SIZ');
      SQL.Add('select #CLZLKC_OutsoleK.CLBH,CLZL.YWPM, SUM(Qty) as Total,CLZL.DWBH,');
      for i:=0 to High(XXCC) do
      begin
        SQL.Add('   Sum(case when SIZ='''+XXCC[i]+''' then Qty end) as ''['+XXCC[i]+']'', ');
      end;
      SQL.Add('CLZL.CQDH ');
      ...
      if EditMatNM.Text<>'' then
        SQL.Add('and CLZL.YWPM like ''%'+EditMatNM.Text+'%'' ');
      SQL.Add('Group by #CLZLKC_OutsoleK.CLBH,CLZL.YWPM,CLZL.CQDH,CLZL.DWBH ');
      if CheckBox1.Checked then
        SQL.Add('HAVING SUM(Qty) > 0 ');
      //funcObj.WriteErrorLog(sql.Text);
      Active:=true;
 ...
 begin
   ...
   begin
      ...
      sql.add('where OSRKSS.RKNO=OSRK.RKNO  and OSRK.CKBH='''+CBX1.Text+''' and OSRKSS.ZLBH=''Outsole'' and isnull(OSRK.flowflag,'''') <>''X'' ');
      sql.add('and convert(smalldatetime,convert(varchar,OSRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
  ...
  begin
   ...
   begin
      ...
      SQL.Add('   select OSLL.CKBH,OSLLSS.CLBH,OSLLSS.SIZ,SUM(OSLLSS.Qty) as Qty into #CLZLKC_BO from OSLLSS,OSLL ');
      SQL.Add('   where OSLLSS.LLNO=OSLL.LLNO  and OSLL.CFMID<>''NO'' and OSLL.SFL in (''2'') and OSLL.CKBH in (Select CKBH from LastKCCK where LB=''05'' and GSBH='''+main.Edit2.Text+''') and isnull(OSLL.flowflag,'''') <>''X'' ');
      SQL.Add('   and convert(smalldatetime,convert(varchar,OSLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
        SQL.Add(' and OSLL.CKBH='''+CBX1.Text+''' ');
      SQL.Add('   Group by OSLL.CKBH,OSLLSS.CLBH,OSLLSS.SIZ ');
...
//
procedure TOutsoleSTOCK.DBGridInit();
var i:integer;
    objColumn:TColumnEh;
begin
    //
    ...
    begin
      Active:=false;
      SQL.Clear;
      SQL.Add('Select Siz from OutsoleSizeR group by Siz order by Siz ');
      Active:=true;
...
begin
 //if  Messagedlg(Pchar('Are you sure caluate Monthly Ending?'),mtInformation,[mbYES,mbNo],0)=mrYes then
  ...
  if CheckOutsoleMonthEnding()=true then
  begin
    if  Messagedlg(Pchar('Are you sure caluate Monthly Ending?'),mtInformation,[mbYES,mbNo],0)=mrYes then
      IsCal:=true
    else
  ...
  begin
      ...
      begin
        Active:=false;
        SQL.Clear;
        SQL.Add('Delete from OutsoleMONTH where KCYear='''+LastYear+''' and KCMonth='''+LastMon+''' and CKBH='''+CBX1.Text+''' ');
        ...
        SQL.Add('  select CKBH,CLBH,SIZ,Sum(RKQty) as NowRKQty from (');
        SQL.Add('  select OSRK.CKBH,OSRKSS.CLBH,OSRKSS.SIZ,SUM(OSRKSS.Qty) as RKQty from OSRKSS,OSRK ');
        SQL.Add('  where OSRKSS.RKNO=OSRK.RKNO  and OSRK.CKBH='''+CBX1.Text+''' and isnull(OSRK.flowflag,'''') <>''X'' and convert(smalldatetime,convert(varchar,OSRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('  Group by OSRK.CKBH,OSRKSS.CLBH,OSRKSS.SIZ ');
        SQL.Add('  Union All ');
        ...
        SQL.Add('left join ( ');
        SQL.Add('   select OSLL.CKBH,OSLLSS.CLBH,OSLLSS.SIZ,SUM(OSLLSS.Qty) as LLQty from OSLLSS,OSLL ');
        SQL.Add('   where OSLLSS.LLNO=OSLL.LLNO  and OSLL.CFMID<>''NO'' and isnull(OSLL.flowflag,'''') <>''X'' and OSLL.CKBH='''+CBX1.Text+''' and convert(smalldatetime,convert(varchar,OSLL.CFMDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('   Group by OSLL.CKBH,OSLLSS.CLBH,OSLLSS.SIZ ) LL on NowKC.CKBH=LL.CKBH and NowKC.CLBH=LL.CLBH and NowKC.SIZ=LL.SIZ');
        //funcObj.WriteErrorLog(sql.Text);
        ExecSQL();
        //月結全部庫存
        Active:=false;
        SQL.Clear;
        SQL.Add('Delete from OutsoleMONTH where KCYear='''+LastYear+''' and KCMonth='''+LastMon+''' and CKBH='''+CBX1.Text+'#L'' ');
        SQL.Add('Insert into OutsoleMONTH ');
        //20161101  IsNull(NowKC.NowRKQty,0) -> IsNull(Sum(NowKC.NowRKQty),0)
        SQL.Add('Select '''+LastYear+''' as KCYear,'''+LastMon+''' as KCMonth,'''+CBX1.Text+'#L'',NowKC.CLBH,NowKC.SIZ,sum(IsNull(NowKC.NowRKQty,0)-IsNull(LL.LLQty,0)),'''+main.Edit1.Text+''',GetDate(),''1''  from (');
        SQL.Add('  select CKBH,CLBH,SIZ,Sum(RKQty) as NowRKQty from (');
        SQL.Add('  select OSRK.CKBH,OSRKSS.CLBH,OSRKSS.SIZ,SUM(OSRKSS.Qty) as RKQty from OSRKSS,OSRK ');
        SQL.Add('  where OSRKSS.RKNO=OSRK.RKNO  and OSRK.CKBH='''+CBX1.Text+''' and isnull(OSRK.flowflag,'''') <>''X'' and OSRKSS.ZLBH=''NEWL'' and convert(smalldatetime,convert(varchar,OSRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('  Group by OSRK.CKBH,OSRKSS.CLBH,OSRKSS.SIZ ');
        SQL.Add('  Union All ');
        ...
        SQL.Add('  select OSLL.CKBH,OSLLSS.CLBH,OSLLSS.SIZ,SUM(OSLLSS.Qty) as LLQty from OSLLSS,OSLL ');
        SQL.Add('  where OSLLSS.LLNO=OSLL.LLNO  and OSLL.CFMID<>''NO'' and OSLL.CKBH='''+CBX1.Text+''' and isnull(OSLL.flowflag,'''') <>''X'' and OSLL.SFL=''2'' ');
        SQL.Add('  and convert(smalldatetime,convert(varchar,OSLL.CFMDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('   Group by OSLL.CKBH,OSLLSS.CLBH,OSLLSS.SIZ ) LL on  NowKC.CLBH=LL.CLBH and NowKC.SIZ=LL.SIZ');
        ...
        sql.add('select DepID,CLBH,SIZ,Sum(LLQty) as NowLLQty from (');
        sql.add('select OSLL.DepID,OSLLSS.CLBH,OSLLSS.SIZ,SUM(OSLLSS.Qty) as LLQty from OSLLSS,OSLL ');
        sql.add('where OSLLSS.LLNO=OSLL.LLNO  and OSLL.CFMID<>''NO'' and OSLL.CKBH='''+CBX1.Text+''' and isnull(OSLL.flowflag,'''') <>''X'' and convert(smalldatetime,convert(varchar,OSLL.CFMDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+'''  ');
        sql.add('Group by OSLL.DepID,OSLLSS.CLBH,OSLLSS.SIZ ');
        sql.add('Union All ');
        ...
        sql.add('group by DepID,CLBH,SIZ ) as NowKC ');
        sql.add('left join ( select OSRK.DepID,OSRKSS.CLBH,OSRKSS.SIZ,SUM(OSRKSS.Qty) as RKQty from OSRKSS,OSRK  ');
        sql.add('where OSRKSS.RKNO=OSRK.RKNO  and OSRK.CKBH='''+CBX1.Text+''' and isnull(OSRK.flowflag,'''') <>''X'' and OSRKSS.ZLBH=''Outsole'' and convert(smalldatetime,convert(varchar,OSRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        sql.add('Group by OSRK.DepID,OSRKSS.CLBH,OSRKSS.SIZ )  RK on NowKC.DepID=RK.DepID and NowKC.CLBH=RK.CLBH and NowKC.SIZ=RK.SIZ ');
        sql.add('where  IsNull(NowKC.NowLLQty,0)-IsNull(RK.RKQty,0) >0 ');
        //sql.Add('group by NowKC.DepID,NowKC.CLBH,NowKC.SIZ');
        //funcObj.WriteErrorLog(sql.Text);
        ExecSQL();
      end;
...
procedure TOutsoleSTOCK.DetailRK1Click(Sender: TObject);
begin
  Outsolestock_RK:=TOutsolestock_RK.create(self);
  Outsolestock_RK.show;
end;

procedure TOutsoleSTOCK.S2Click(Sender: TObject);
begin
  OutsoleStock_Detail:=TOutsoleStock_Detail.create(self);
  OutsoleStock_Detail.show;
end;

procedure TOutsoleSTOCK.MenuItem1Click(Sender: TObject);
begin
  Outsolestock_LL:=TOutsolestock_LL.create(self);
  Outsolestock_LL.show;
end;

procedure TOutsoleSTOCK.BDT5Click(Sender: TObject);
begin
  with Qtemp1 do
  begin
    active:=false;
    sql.Clear;
    sql.add('select CKBH,Memo from LastKCCK WHERE GSBH='''+main.Edit2.text+''' and CKBH ='''+CBX1.text+''' and LB =''05'' order by CKBH');
    active:=true;
  end;
  OutsoleSTOCK_PRINT:=TOutsoleSTOCK_PRINT.Create(self);
  OutsoleSTOCK_PRINT.quickrep1.prepare;
  OutsoleSTOCK_PRINT.QRLabel39.Caption:=Qtemp1.FieldByName('CKBH').AsString+'-'+Qtemp1.FieldByName('Memo').AsString;
  OutsoleSTOCK_PRINT.QRLabel35.Caption:=main.Edit2.text;
  OutsoleSTOCK_PRINT.PageN.caption:=inttostr(OutsoleSTOCK_PRINT.quickrep1.QRPrinter.pagecount);
  OutsoleSTOCK_PRINT.quickrep1.preview;
  OutsoleSTOCK_PRINT.free;
end;

procedure TOutsoleSTOCK.BitBtn1Click(Sender: TObject);
begin
  with Qtemp1 do
  begin
    active:=false;
  ...
  OutsoleSTOCK_PRINT_Tong:=TOutsoleSTOCK_PRINT_Tong.Create(self);
  OutsoleSTOCK_PRINT_Tong.quickrep1.prepare;
  OutsoleSTOCK_PRINT_Tong.QRLabel39.Caption:=Qtemp1.FieldByName('CKBH').AsString+'-'+Qtemp1.FieldByName('Memo').AsString;
  OutsoleSTOCK_PRINT_Tong.QRLabel35.Caption:=main.Edit2.text;
  OutsoleSTOCK_PRINT_Tong.PageN.caption:=inttostr(OutsoleSTOCK_PRINT_Tong.quickrep1.QRPrinter.pagecount);
  OutsoleSTOCK_PRINT_Tong.quickrep1.preview;
  OutsoleSTOCK_PRINT_Tong.free;
end;
