...
begin
  ...
  begin
    ...
    SQL.Add(',case when TCLYL_M is not null then Qty/TCLYL_M*TCLYL else 0 end as Qty,case when TCLYL_M is not null then TempQty/TCLYL_M*TCLYL else 0 end as TempQty,DAOMH ');
    SQL.Add('from ( ');
    ...
    SQL.Add('case when Substring(ZLZLS3.ZLBH1,4,1)=''V'' then YPZL_XXZL.DAOMH else XXZL.DAOMH end as DAOMH');
    ...
    SQL.Add('case when Substring(ZLZLS3.ZLBH1,4,1)=''V'' then YPZL_XXZL.DAOMH else XXZL.DAOMH end');
    //
    ...
    SQL.Add('       case when Substring(KCLLS.SCBH,4,1)=''V'' then YPZL_XXZL.DAOMH else XXZL.DAOMH end as DAOMH  ');
    SQL.Add('from  KCLLS');
    ...
    SQL.Add('case when Substring(KCLLS.SCBH,4,1)=''V'' then YPZL_XXZL.DAOMH else XXZL.DAOMH end');
...
begin
  ...
  begin
    active:=false;
    sql.Clear;
    sql.Add('Select cldh,DDBH,cldhz,ZLBH1,TCLYL,zwpm,ywpm,CQDH,Qty,TempQty,DWBH,Article,XieMing,case when Substring(ZLBH1,1,3)=''CBY'' then PairCBY else Pairs end as Pairs, CalDate,THQty,DAOMH ');
    sql.Add('from(');
    sql.Add('SELECT max(ZLZLS3.cldh) as cldh,max(ZLZLS3.DDBH) as DDBH,right(ZLZLS3.cldhz,10) as cldhz,ZLZLS3.ZLBH1 as ZLBH1,Convert(float,round(sum(ZLZLS3.TCLYL),2)) as TCLYL,');
    sql.Add('CLZL.zwpm,CLZL.ywpm,ZLZLS3.CQDH ,isnull(KCLLS.Qty,0) as Qty,isnull(KCLLS.TempQty,0) as TempQty,CLZL.DWBH,');
    //20170623
    ...
    sql.Add('case when Substring(ZLZLS3.ZLBH1,4,1)=''V'' then YPZL_XXZL.XieMing else XXZL.XieMing end as XieMing,');
    sql.Add('case when Substring(ZLZLS3.ZLBH1,4,1)=''V'' then YPZL.Quantity else ERP_DDZL.Pairs end as Pairs,');
    SQL.Add('case when Substring(ZLZLS3.ZLBH1,4,4)=''V'' then YPZL_XXZL.DAOMH else XXZL.DAOMH end as DAOMH,');
    ...
    sql.Add('case when Substring(ZLZLS3.ZLBH1,4,1)=''V'' then YPZL_XXZL.XieMing else XXZL.XieMing end,');
    sql.Add('case when Substring(ZLZLS3.ZLBH1,4,1)=''V'' then YPZL.Quantity else ERP_DDZL.Pairs  end, KCRKS.Qty,');
    SQL.Add('case when Substring(ZLZLS3.ZLBH1,4,4)=''V'' then YPZL_XXZL.DAOMH else XXZL.DAOMH end');
    sql.Add(')ZLZLS3');
    //
    if PerCombo.ItemIndex=1 then
      sql.add('where ISNULL(Qty / NULLIF(TCLYL, 0), 0)>1.00')
