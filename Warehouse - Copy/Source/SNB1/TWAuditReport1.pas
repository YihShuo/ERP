...
type
  TTWAuditReport = class(TForm)
    ...
    Query4ActQty: TCurrencyField;
    Query4ZMLB: TStringField;
...
begin
  ...
  begin
    ...
    begin
      ...
      sql.add('KCLL.MEMO,KCLLS.MEMO as Note,KCLL.CKBH from KCLLS');
  ...
  begin
    ...
    begin
      active:=false;
      sql.Clear;
      sql.add(' select JGZL.CFMDate1,JGZLS.JGNO,JGZLS.CLBH,JGZLS.ZMLB,clzl.ywpm,clzl.dwbh');
      sql.Add('       ,case when ZMLB <>''ZZZZZZZZZZ'' then JGZLS.Qty else  '''' end as Qty ');
      sql.Add('       ,case when ZMLB <>''ZZZZZZZZZZ'' then JGZLS.Qty*JGZLM.Qty else JGZLS.Qty end as ActQty ');
      ...
      sql.add(' left join CLZL on CLZL.cldh = case when JGZLS.ZMLB=''ZZZZZZZZZZ'' then JGZLS.CLBH else JGZLS.ZMLB end  ');
      sql.add(' left join zszl on zszl.zsdh = JGZL.ZSBH ');
      sql.Add(' left join ( select JGZLS.Qty,JGZLS.JGNO,JGZLS.CLBH ');
      sql.Add('             from JGZLS  ');
      ...
      sql.add('          '''+formatdatetime('yyyy/MM/dd',DTP1.Date) +''''+' and '+''''+formatdatetime('yyyy/MM/dd',DTP2.Date) +'''');
      sql.add('order by JGZL.CFMDate1,JGZL.JGNO,JGZLS.CLBH,JGZLS.ZMLB ');
      active:=true;
    end;
  end;
end;
