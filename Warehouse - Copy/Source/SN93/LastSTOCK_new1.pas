...
begin
 ...
 begin
   ...
   begin
      ...
      SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and LTRK.SFL in (''1'',''2'') and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
        SQL.Add('  and LTRK.CKBH='''+CBX1.Text+''' ');
      SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
      SQL.Add('  Union All ');
      SQL.Add('  select CKBH,CLBH,SIZ,QTY as LastRem from Lastmonth where KCYEAR='''+ayear+''' and KCMonth='''+amonth+''' ');
      if CBX1.Text <> '' then
      ...
      SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.SFL in (''1'',''2'')  and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
 ...
 begin
   ...
   begin
      ...
      SQL.Add('  select CKBH,CLBH,SIZ,Sum(RKQty) as NowRKQty from (');
      SQL.Add('  select LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK ');
      SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO and LTRK.SFL=''2'' and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
        SQL.Add(' and LTRK.CKBH='''+CBX1.Text+''' ');
      SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
      SQL.Add('  Union All ');
      ...
      SQL.Add('left join ( ');
      SQL.Add('   select LTLL.CKBH,LTLLSS.CLBH,LTLLSS.SIZ,SUM(LTLLSS.Qty) as LLQty from LTLLSS,LTLL ');
      SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.SFL=''2'' and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
      if CBX1.Text <> '' then
        SQL.Add(' and LTLL.CKBH='''+CBX1.Text+''' ');
      SQL.Add('   Group by LTLL.CKBH,LTLLSS.CLBH,LTLLSS.SIZ ) LL on NowKC.CKBH=LL.CKBH and NowKC.CLBH=LL.CLBH and NowKC.SIZ=LL.SIZ');
 ...
 begin
   ...
   begin
      ...
      sql.add('where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.SFL=''1''  and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+'''  ');
      if CBX1.Text <> '' then
        SQL.Add(' and LTLL.CKBH='''+CBX1.Text+''' ');
      sql.add('Group by LTLL.DepID,LTLLSS.CLBH,LTLLSS.SIZ ');
      sql.add('Union All ');
      sql.add('select DepID,CLBH,SIZ,QTY as LastRem from Lastmonth_Out where KCYEAR='''+ayear+''' and KCMonth='''+amonth+''' ');
      if CBX1.Text <> '' then
        SQL.Add(' and CKBH='''+CBX1.Text+''' ');
      sql.add(') as tmpRKTLFL ');
      ...
      sql.add('where LTRKSS.RKNO=LTRK.RKNO and LTRK.SFL=''1'' and LTRKSS.ZLBH=''LAST'' and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
        SQL.Add(' and LTRK.CKBH='''+CBX1.Text+''' ');
 ...
 begin
   ...
   begin
      ...
      SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.SFL in (''2'')  and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+formatdatetime('yyyy/MM/dd',startofthemonth(DTP1.date))+''' and '''+formatdatetime('yyyy/MM/dd',DTP1.date)+''' ');
      if CBX1.Text <> '' then
        SQL.Add(' and LTLL.CKBH='''+CBX1.Text+''' ');
...
begin
  ...
  begin
      ...
      begin
        Active:=false;
        SQL.Clear;
        SQL.Add('Delete from LASTMONTH where KCYear='''+LastYear+''' and KCMonth='''+LastMon+''' and CKBH='''+CBX1.Text+''' ');
        ...
        SQL.Add('  select CKBH,CLBH,SIZ,Sum(RKQty) as NowRKQty from (');
        SQL.Add('  select LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK ');
        SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH='''+CBX1.Text+''' and LTRK.SFL in (''1'',''2'') and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
        SQL.Add('  Union All ');
        ...
        SQL.Add('left join ( ');
        SQL.Add('   select LTLL.CKBH,LTLLSS.CLBH,LTLLSS.SIZ,SUM(LTLLSS.Qty) as LLQty from LTLLSS,LTLL ');
        SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.CKBH='''+CBX1.Text+''' and LTLL.SFL in (''1'',''2'') and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('   Group by LTLL.CKBH,LTLLSS.CLBH,LTLLSS.SIZ ) LL on NowKC.CKBH=LL.CKBH and NowKC.CLBH=LL.CLBH and NowKC.SIZ=LL.SIZ');
        ExecSQL();
        //月結全部庫存
        Active:=false;
        SQL.Clear;
        SQL.Add('Delete from LASTMONTH where KCYear='''+LastYear+''' and KCMonth='''+LastMon+''' and CKBH='''+CBX1.Text+'#L'' ');
        SQL.Add('Insert into LASTMONTH ');
        ...
        SQL.Add('  select CKBH,CLBH,SIZ,Sum(RKQty) as NowRKQty from (');
        SQL.Add('  select LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK ');
        SQL.Add('  where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH='''+CBX1.Text+''' and LTRK.SFL=''2'' and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('  Group by LTRK.CKBH,LTRKSS.CLBH,LTRKSS.SIZ ');
        SQL.Add('  Union All ');
        ...
        SQL.Add('left join ( ');
        SQL.Add('   select LTLL.CKBH,LTLLSS.CLBH,LTLLSS.SIZ,SUM(LTLLSS.Qty) as LLQty from LTLLSS,LTLL ');
        SQL.Add('   where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.CKBH='''+CBX1.Text+''' and LTLL.SFL=''2'' and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        SQL.Add('   Group by LTLL.CKBH,LTLLSS.CLBH,LTLLSS.SIZ ) LL on NowKC.CKBH=LL.CKBH and NowKC.CLBH=LL.CLBH and NowKC.SIZ=LL.SIZ');
       // sql.Add('  group by NowKC.CLBH,NowKC.SIZ');
        ...
        sql.add('select DepID,CLBH,SIZ,Sum(LLQty) as NowLLQty from (');
        sql.add('select LTLL.DepID,LTLLSS.CLBH,LTLLSS.SIZ,SUM(LTLLSS.Qty) as LLQty from LTLLSS,LTLL ');
        sql.add('where LTLLSS.LLNO=LTLL.LLNO  and LTLL.CFMID<>''NO'' and LTLL.CKBH='''+CBX1.Text+''' and LTLL.SFL=''1'' and convert(smalldatetime,convert(varchar,LTLL.CFMDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+'''  ');
        sql.add('Group by LTLL.DepID,LTLLSS.CLBH,LTLLSS.SIZ ');
        sql.add('Union All ');
        ...
        sql.add('group by DepID,CLBH,SIZ ) as NowKC ');
        sql.add('left join ( select LTRK.DepID,LTRKSS.CLBH,LTRKSS.SIZ,SUM(LTRKSS.Qty) as RKQty from LTRKSS,LTRK  ');
        sql.add('where LTRKSS.RKNO=LTRK.RKNO  and LTRK.CKBH='''+CBX1.Text+''' and LTRK.SFL=''1'' and convert(smalldatetime,convert(varchar,LTRK.UserDate,111)) between '''+STLastMonDate+''' and '''+EDLastMonDate+''' ');
        sql.add('Group by LTRK.DepID,LTRKSS.CLBH,LTRKSS.SIZ )  RK on NowKC.DepID=RK.DepID and NowKC.CLBH=RK.CLBH and NowKC.SIZ=RK.SIZ ');
        sql.add('where  IsNull(NowKC.NowLLQty,0)-IsNull(RK.RKQty,0) >0 ');
        ExecSQL();
      end;
